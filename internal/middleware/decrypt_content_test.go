package middleware

import (
	"bytes"
	"github.com/e1m0re/grdn/internal/service/encryption"
	"github.com/go-chi/chi/v5"
	"github.com/stretchr/testify/require"
	"net/http"
	"net/http/httptest"
	"os"
	"testing"
)

const (
	publicKey  = "-----BEGIN RSA PUBLIC KEY-----\\n-----END RSA PUBLIC KEY-----"
	privateKey = "-----BEGIN RSA PRIVATE KEY-----\\n-----END RSA PRIVATE KEY-----\n"
)

func TestDecryptContent(t *testing.T) {
	err := os.WriteFile("/tmp/TestDecryptContent_private_key", []byte(privateKey), 0666)
	if err != nil {
		panic(err)
	}
	err = os.WriteFile("/tmp/TestDecryptContent_public_key", []byte(publicKey), 0666)
	if err != nil {
		panic(err)
	}

	type args struct {
		privateKeyFile string
		method         string
		body           []byte
		encryptBody    bool
	}
	type want struct {
		body       []byte
		statusCode int
	}
	tests := []struct {
		name string
		want want
		args args
	}{
		{
			name: "invalid private key",
			args: args{
				privateKeyFile: "/tmp/TestDecryptContent_private_key_invalid",
				body:           []byte("hello world!"),
				encryptBody:    true,
				method:         "POST",
			},
			want: want{
				statusCode: http.StatusInternalServerError,
				body:       []byte("hello world!"),
			},
		},
		{
			name: "successfully case (GET)",
			args: args{
				privateKeyFile: "/tmp/TestDecryptContent_private_key",
				body:           []byte("hello world!"),
				encryptBody:    true,
				method:         "GET",
			},
			want: want{
				statusCode: http.StatusOK,
				body:       []byte("hello world!"),
			},
		},
		{
			name: "successfully case (POST)",
			args: args{
				privateKeyFile: "/tmp/TestDecryptContent_private_key",
				body:           []byte("hello world!"),
				encryptBody:    true,
				method:         "POST",
			},
			want: want{
				statusCode: http.StatusOK,
				body:       []byte("hello world!"),
			},
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			recorder := httptest.NewRecorder()

			r := chi.NewRouter()
			r.Use(DecryptContent(test.args.privateKeyFile))
			r.Post("/", func(w http.ResponseWriter, r *http.Request) {})
			r.Get("/", func(w http.ResponseWriter, r *http.Request) {})

			body := test.args.body
			if test.args.encryptBody {
				encr, _ := encryption.NewEncryptor("/tmp/TestDecryptContent_public_key")
				body, _ = encr.Encrypt(test.args.body)
			}
			request := httptest.NewRequest(test.args.method, "/", bytes.NewReader(body))

			r.ServeHTTP(recorder, request)
			response := recorder.Result()

			require.Equal(t, test.want.statusCode, response.StatusCode)

			response.Body.Close()
		})
	}
}
