...
func Test_decryptor_Decrypt(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "private key not specified",
			fields: fields{
				publicKey: func() *rsa.PublicKey {
			...
			want: want{
				result: nil,
				err:    errors.New("RSA private key not specified"),
			},
		},
		{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if err == nil {
				assert.Equal(t, test.args.plaintext, got)
			}
		})
	}
}
...
func TestNewDecryptor(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				keyFile: "/tmp/grdn_key_test_decryptor",
			},
			fields: fields{
				createFile: false,
			},
			want: want{
				...
				err: &fs.PathError{
					Op:   "open",
					Path: "/tmp/grdn_key_test_decryptor",
					Err:  syscall.ENOENT,
				},
			},
		},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			got, err := NewDecryptor(test.args.keyFile)
			assert.Equal(t, test.want.err, err)
			if test.want.err != nil {
				assert.Nil(t, got)
			}
			if test.want.decryptor != nil {
				assert.Implements(t, (*Decryptor)(nil), got)
			}
		})
	}
}
